...
Currently using MPO, JPG, JPEG or JPS extensions only.
"""
...
def get_file_list(exts=["jpg", " jpeg", "jps"]):
    exl = []
    for ext in exts:
        if ext.lower() not in exl:
            exl.append(ext.lower())
        if ext.upper() not in exl:
            exl.append(ext.upper())
...
def do_menu():
    ...
    while ch != 'x':
        ...
        print("e. Pad to square (1:1) aspect ratio.")
        print("f. Pad to double square (2:1) aspect ratio.")
        print("x. Exit.")
        print()
